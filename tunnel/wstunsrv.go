...
func NewWSTunnelServer(args []string) *WSTunnelServer {
	...
	srvFlag.IntVar(&wstunSrv.MaxRequestsPerTunnel, "max-requests-per-tunnel", defaultMaxReq, "maximum number of queued requests per tunnel")
	srvFlag.IntVar(&wstunSrv.MaxClientsPerToken, "max-clients-per-token", 0, "maximum number of clients per token (0 for unlimited)")
	...
	if wstunSrv.MaxRequestsPerTunnel < 0 {
		...
		wstunSrv.Log.Warn("max-requests-per-tunnel is 0, this will prevent any requests from being processed", "value", wstunSrv.MaxRequestsPerTunnel)
	} else if wstunSrv.MaxRequestsPerTunnel > 1000 {
		wstunSrv.Log.Warn("max-requests-per-tunnel is very high, may cause resource issues", "value", wstunSrv.MaxRequestsPerTunnel)
	}
...
func (t *WSTunnelServer) getRemoteServer(tok token, create bool) *remoteServer {
	...
	rs = &remoteServer{
		...
		requestQueue: make(chan *remoteRequest, t.MaxRequestsPerTunnel),
