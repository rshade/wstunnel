...
// TestMaxClientsPerToken tests the max clients per token configuration
func TestMaxClientsPerToken(t *testing.T) {
	// Create a test server with max clients per token limit
	ts := &WSTunnelServer{
		Log:                log15.Root(),
		MaxClientsPerToken: 2, // Allow only 2 clients per token
	...
	// Test adding clients up to the limit
	// Simulate first client connection
	ts.tokenClientsMutex.Lock()
	ts.tokenClients[testToken] = 1
	ts.tokenClientsMutex.Unlock()

	// Check current count
	ts.tokenClientsMutex.RLock()
	count := ts.tokenClients[testToken]
	ts.tokenClientsMutex.RUnlock()
	if count != 1 {
		t.Errorf("Expected client count to be 1, got %d", count)
	}

	// Simulate second client connection
	ts.tokenClientsMutex.Lock()
	currentClients := ts.tokenClients[testToken]
	if currentClients < ts.MaxClientsPerToken {
		ts.tokenClients[testToken] = currentClients + 1
	}
	ts.tokenClientsMutex.Unlock()
	...
	ts.tokenClientsMutex.RUnlock()
	if count != 2 {
		t.Errorf("Expected client count to be 2, got %d", count)
	}

	// Try to add third client, should be rejected
	ts.tokenClientsMutex.Lock()
	currentClients = ts.tokenClients[testToken]
	shouldReject := currentClients >= ts.MaxClientsPerToken
	ts.tokenClientsMutex.Unlock()

	if !shouldReject {
		t.Error("Expected third client to be rejected when limit is reached")
	}

	// Test cleanup when client disconnects
	ts.tokenClientsMutex.Lock()
	if count, exists := ts.tokenClients[testToken]; exists && count > 0 {
		ts.tokenClients[testToken] = count - 1
		if ts.tokenClients[testToken] == 0 {
			delete(ts.tokenClients, testToken)
		}
	}
	ts.tokenClientsMutex.Unlock()
	...
	ts.tokenClientsMutex.RUnlock()
	if count != 1 {
		t.Errorf("Expected client count to be 1 after disconnect, got %d", count)
	}
}
